...
const InvestmentSummary: React.FC = memo(() => {
  const { balanceData, isLoading: balanceLoading } = useBalance();
  const [summaryCards, setSummaryCards] = useState<SummaryCardData[]>([]);

  useEffect(() => {
    if (!balanceData) return;

    const { summary, positions } = balanceData;
    ...
    const cards: SummaryCardData[] = [
      ...
      },
      {
        ...
        subValue: `(${dailyProfitRate > 0 ? "+" : ""}${dailyProfitRate.toFixed(
          1
        )}%)`,
        changeClass:
          dailyProfitRate > 0
